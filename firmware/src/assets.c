...
void setNote(unsigned short freq, unsigned short dur) {
   ...
   if (dur <= freq) dur = (freq << 1); /* too short to be played with PWM, so double it */
...
void nextNote_cb(unsigned char assetId, int frame) 
{
   ...
   if (G_currentNote < assetList[assetId].x) {
	...
	if (G_duration <= G_freq) G_duration = (G_freq << 1); /* too short to be played with PWM, so double it */
#ifdef DEBUGAUDIO
{
